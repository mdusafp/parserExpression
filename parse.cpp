...
void wrapperOfCalculation(string &exp, string &leftOp, string &rightOp, vector<int> &signI, 
                          int &begL, int &endL, int &begR, int &endR, char prevS) {
...
void searchOperand(string &exp, int &endLeftOp, int &begRightOp, int &endRightOp, 
                   char &prevS, int indexLeftS, int indexRightS) {
        ...
        if (exp[i] == '+' || exp[i] == '-' || exp[i] == '/' || exp[i] == '*') {
            ...
            if (signIndex.size() > 1) { // when we find two sign (always find <= 2)
                ...
                if (prevSign == '*' || prevSign == '/' || Sign == '+' || Sign == '-') {
                    // if we have exp like 12*-1 
                    if (exp[i] == '-' && 
                        signIndex [ signIndex.size() - 1 ] - signIndex [ signIndex.size() - 2 ] == 1) {
                        signIndex.pop_back();
                    } else {
                        // search index the start and end indices of left and right operands
                        searchOperand(exp, endLeftOperand, begRightOperand, endRightOperand, 
                                      prevSign, signIndex [ signIndex.size() - 2 ], 
                                      signIndex [ signIndex.size() - 1 ]);
                        // wrapper of calculation
                        wrapperOfCalculation(exp, leftOperand, rightOperand, signIndex, 
                                             begLeftOperand, endLeftOperand, begRightOperand, 
                                             endRightOperand, prevSign);
                        i = 0;
                        signIndex.clear();
                        begLeftOperand = 0;
                    }
            ...
            searchOperand(exp, endLeftOperand, begRightOperand, endRightOperand, 
                          prevSign, signIndex[0], exp.length());
            wrapperOfCalculation(exp,leftOperand, rightOperand, signIndex, 
                                 begLeftOperand, endLeftOperand, begRightOperand, endRightOperand, prevSign);
        }
        if (i == exp.length() - 1 && signIndex.size() == 2) {
            searchOperand(exp, endLeftOperand, begRightOperand, endRightOperand, prevSign, signIndex[1], 
                          exp.length());

            wrapperOfCalculation(exp,leftOperand, rightOperand, signIndex, begLeftOperand, endLeftOperand, 
                                 begRightOperand, endRightOperand, prevSign);
            i = 0;
            signIndex.clear();
            begLeftOperand = 0;
...
string parser(string &exp) {
    ...
    if (exp[0] == ')' ||
        exp[0] == '*' ||
        exp[0] == '/') {
        cout << "It's too hard to calculate;(\n";
        calced = false;
        return "";
        // exit(EXIT_FAILURE);
    }
...
void fileProcessing(string &exp) {
    ifstream in("/home/mfp/cpp/cursach/Easy.txt");
    if (!in.is_open()) {
